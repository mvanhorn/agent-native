...
## 带走你的资源 {#resource-pack}

导出一份带校验和的 JSON 包，包含你已经能读取的文本资源（`export-resource-pack`），再导入到另一个应用或用户（`import-resource-pack`）。资源选项卡上的 **Export pack** / **Import pack** 调用的是同一组操作。

- 导出只包含文本。二进制上传会被省略。形如 API 密钥的字符串以及 MCP 的 `env` / `headers` 令牌字段会被脱敏；导入后请重新连接 MCP。
- 导入默认写入 **个人** 范围，冲突时跳过。组织导入需要与编辑组织文件相同的管理员写入 ACL。工作区导入不可用。
- 超过 200 个文本文件或 1 MB 的包会以 `too_large` 失败，而不是截断。校验和不匹配会失败关闭。

...
## リソースを持ち運ぶ {#resource-pack}

すでに読めるテキストリソースのチェックサム付き JSON パックを書き出し（`export-resource-pack`）、別のアプリやユーザーへ取り込みます（`import-resource-pack`）。Resources タブの **Export pack** / **Import pack** は同じアクションを呼び出します。

- 書き出しはテキストのみです。バイナリのアップロードは省略されます。API キー型の文字列と MCP の `env` / `headers` トークン欄は伏せられます。取り込み後は MCP を再接続してください。
- 取り込みの既定先は **personal** スコープで、衝突時はスキップします。組織への取り込みには、組織ファイル編集と同じ管理者書き込み ACL が必要です。ワークスペース取り込みはできません。
- テキスト 200 ファイルまたは 1 MB を超えるパックは切り詰めず `too_large` で失敗します。チェックサム不一致は閉じて失敗します。

...
## Emporter vos ressources {#resource-pack}

Exportez un pack JSON avec somme de contrôle des ressources texte que vous pouvez déjà lire (`export-resource-pack`), puis importez-le dans une autre app ou un autre utilisateur (`import-resource-pack`). Les commandes **Export pack** / **Import pack** de l’onglet Ressources appellent les mêmes actions.

- L’export n’inclut que du texte. Les téléversements binaires sont omis. Les chaînes en forme de clé API et les champs de jeton MCP `env` / `headers` sont masqués ; reconnectez MCP après l’import.
- L’import cible par défaut la portée **personnel** et ignore les conflits. L’import d’organisation exige la même ACL d’écriture admin que l’édition des fichiers d’organisation. L’import d’espace de travail n’est pas disponible.
- Un pack de plus de 200 fichiers texte ou 1 Mo échoue avec `too_large` au lieu d’être tronqué. Une somme de contrôle incorrecte échoue de façon fermée.

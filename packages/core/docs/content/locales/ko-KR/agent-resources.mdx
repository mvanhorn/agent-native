...
## 리소스를 가져가세요 {#resource-pack}

이미 읽을 수 있는 텍스트 리소스의 체크섬 JSON 팩을 내보내고(`export-resource-pack`) 다른 앱이나 사용자로 가져옵니다(`import-resource-pack`). Resources 탭의 **Export pack** / **Import pack** 은 같은 액션을 호출합니다.

- 내보내기는 텍스트만 포함합니다. 바이너리 업로드는 생략됩니다. API 키 형태의 문자열과 MCP `env` / `headers` 토큰 필드는 가려집니다. 가져온 뒤 MCP를 다시 연결하세요.
- 가져오기는 기본이 **personal** 범위이며 충돌 시 건너뜁니다. 조직 가져오기는 조직 파일 편집과 같은 관리자 쓰기 ACL이 필요합니다. 워크스페이스 가져오기는 불가합니다.
- 텍스트 파일 200개 또는 1MB를 넘는 팩은 자르지 않고 `too_large` 로 실패합니다. 체크섬 불일치는 닫힌 실패입니다.

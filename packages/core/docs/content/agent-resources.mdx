...
## Take resources with you {#resource-pack}

Export a checksummed JSON pack of the text resources you can already read (`export-resource-pack`), and import it into another app or user (`import-resource-pack`). The Resources tab **Export pack** / **Import pack** controls call the same actions.

- Export includes only text. Binary uploads are omitted. API-key-shaped strings and MCP `env` / `headers` token fields are redacted; reconnect MCP after import.
- Import defaults to **personal** scope with skip-on-conflict. Organization import requires the same admin write ACL as editing organization files. Workspace import is not available.
- A pack that exceeds 200 text files or 1 MB fails with `too_large` instead of truncating. A checksum mismatch fails closed.
